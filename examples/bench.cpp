...
int main() {
    ...
    std::atomic<std::uint32_t> pairs;
    {
        ...
        spdlog::error("Elapsed for CLL {}ms = {} * {}ms", elapsed.count(), nJobs, elapsed.count() / static_cast<float>(nJobs));
    }
    ...
    std::uint32_t referencePairs{};
    {
        ...
        auto end = std::chrono::high_resolution_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
        spdlog::error("Elapsed for reference {}ms = {} * {}ms", elapsed.count(), nJobs, elapsed.count() / static_cast<float>(nJobs));
    }
